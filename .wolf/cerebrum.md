...
- **Three local patches ride on top of 2.0.1**: `anatomy-store.js` (bug-008), `shared.js` (bug-009), and the missing `symbol-extractor.js` (bug-007). They are re-applied automatically by `scripts/openwolf-repair.mjs`, wired as a SessionStart hook, so `openwolf update` can no longer silently revert them. Run by hand after an upgrade: `node scripts/openwolf-repair.mjs --verbose`.
