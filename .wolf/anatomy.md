# anatomy.md

> Auto-maintained by OpenWolf. Last scanned: 2026-08-12T06:04:13.262Z
> Files: 80 tracked | Anatomy hits: 0 | Misses: 0

## ./

- `.gitignore` — Git ignore rules (~30 tok)
- `CLAUDE.md` — OpenWolf (~57 tok)
- `desktop.ini` (~29 tok)
- `README.md` — Project documentation (~677 tok)

## .claude/

- `settings.json` (~441 tok)

## .claude/rules/

- `openwolf.md` (~313 tok)

## _private/

- `desktop.ini` (~29 tok)
- `README.md` — Project documentation (~52 tok)

## _private/supabase-sql/

- `ai-agent-repo-operating-guide.insert.sql` — Adds the AI Agent Repo Operating Guide to public.guides. (~1822 tok)
- `desktop.ini` (~29 tok)
- `new-machine-claude-code-setup.insert.sql` — Adds a beginner-first "new machine" setup guide to public.guides. (~2821 tok)
- `openai-codex.github_repos.insert.sql` — Adds the missing OpenAI Codex repo to public.github_repos. (~292 tok)
- `README.md` — Project documentation (~45 tok)
- `recommended-ai-agent-repos.github_repos.insert.sql` — Inserts/updates recommended Claude Code / Codex ecosystem repos for public.github_repos. (~2847 tok)

## _private/supabase-sql/legacy-v0/

- `desktop.ini` (~29 tok)
- `new-machine-claude-code-setup.insert.sql` — Adds the "new machine" guide to public.guides. (~2326 tok)
- `openai-codex.github_repos.insert.sql` — Adds the missing OpenAI Codex repo to public.github_repos. (~292 tok)

## archive/

- `desktop.ini` (~29 tok)
- `README.md` — Project documentation (~54 tok)

## archive/context-notes/

- `desktop.ini` (~29 tok)
- `לאן-להמשיך-מכאן.txt` (~3283 tok)

## archive/legacy-outputs/

- `ai-agent-repo-advisor.en.default.html` — AI Agent Repo Advisor (~4578 tok)
- `desktop.ini` (~29 tok)
- `new-machine-claude-code-setup.he.default.html` — מחשב חדש ל-Claude Code ו-Codex (~3949 tok)

## archive/legacy-outputs/V0/

- `desktop.ini` (~29 tok)
- `new-machine-claude-code-setup.html` — מחשב חדש ל-Claude Code ו-Codex (~2680 tok)
- `new-machine-claude-code-setup.md` — מחשב חדש לעבודה עם Claude Code ו-Codex (~1748 tok)

## current/

- `desktop.ini` (~29 tok)
- `README.md` — Project documentation (~124 tok)

## current/guides/

- `ai-agent-repo-advisor.en.html` — AI Agent Repo Advisor (~4578 tok)
- `ai-agent-repo-advisor.he.html` — יועץ ריפואים לסוכני קוד AI (~3762 tok)
- `ai-agent-repo-operating-guide.md` — AI Agent Repo Operating Guide (~3073 tok)
- `claude-code-composio-mcp.he.html` — חיבור כלים חיצוניים ל-Claude Code עם Composio (~3552 tok)
- `claude-code-why-infrastructure.he.html` — למה Claude Code — כלי פיתוח או תשתית? (~2743 tok)
- `claude-code-workspace-architecture.he.html` — ארכיטקטורת Workspace — Claude Code (~3047 tok)
- `desktop.ini` (~29 tok)
- `google-play-publishing.he.md` — מדריך להעלאת אפליקציה ל-Google Play Store (~2680 tok)
- `new-machine-claude-code-setup.en.html` — New Computer Setup for Claude Code and Codex (~2739 tok)
- `new-machine-claude-code-setup.he.html` — מחשב חדש ל-Claude Code ו-Codex (~4599 tok)
- `new-machine-claude-code-setup.md` — מחשב חדש ל-Claude Code ו-Codex (~2727 tok)

## current/skills/

- `agent-onboarding-wizard.skill` (~608 tok)
- `desktop.ini` (~29 tok)
- `new-project-from-prd.skill` (~721 tok)

## current/skills/agent-onboarding-wizard/

- `desktop.ini` (~29 tok)
- `SKILL.md` — Agent Onboarding Wizard (~1283 tok)

## current/skills/cv-story-builder/

- `desktop.ini` (~29 tok)
- `SKILL.md` — CV Story Builder (~1566 tok)

## current/skills/cv-story-builder/agents/

- `desktop.ini` (~29 tok)
- `openai.yaml` (~82 tok)

## current/skills/cv-story-builder/references/

- `agent-and-evidence-checklist.md` — Agent and Evidence Checklist (~688 tok)
- `desktop.ini` (~29 tok)
- `top-1-superprompt.md` — Top 1% CV Story Builder Superprompt (~474 tok)

## current/skills/google-play-publishing/

- `SKILL.md` — Google Play Publishing (~3011 tok)

## current/skills/improve-codebase-architecture/

- `ATTRIBUTION.md` — Credits the original Matt Pocock skill and lists what this revision changed. Not referenced from SKILL.md, so it costs no runtime context (~350 tok)
- `audit_skill.py` — Audits the skill package against authoring best practices and this skill's own review findings; 45 checks (~2600 tok)
- `HTML-REPORT.md` — Scaffold, card structure, and diagram patterns for the self-contained HTML review report (~1300 tok)
- `INTERFACE-DESIGN.md` — How to generate and pressure-test alternative interface shapes during the design step (~900 tok)
- `LANGUAGE.md` — Architecture vocabulary: module, interface, depth, seam, adapter, leverage, locality, plus the deletion test and words to avoid (~1200 tok)
- `SKILL.md` — Improve Codebase Architecture: surveys a codebase for shallow modules, ranks deepening candidates in a self-contained HTML report, then designs the one the user picks. Proposes only, never edits code (~2100 tok)

## current/skills/improve-codebase-architecture/evals/

- `evals.json` — 4 behavioural evals with assertions: report gate, no-grill request, zero-findings honesty, repo-write consent (~600 tok)

## current/skills/new-project-from-prd/

- `desktop.ini` (~29 tok)
- `SKILL.md` — New Project From PRD (~1524 tok)

## current/skills/outside-reader-audit/

- `desktop.ini` (~29 tok)
- `SKILL.md` — Outside Reader Audit (~1019 tok)

## current/skills/outside-reader-audit/agents/

- `desktop.ini` (~29 tok)
- `openai.yaml` (~104 tok)

## docs/

- `desktop.ini` (~29 tok)
- `publishing-recommendation.md` — Publishing Recommendation (~762 tok)

## examples/

- `desktop.ini` (~29 tok)
- `README.md` — Project documentation (~117 tok)

## examples/lecture-demos/

- `desktop.ini` (~29 tok)
- `README.md` — Project documentation (~47 tok)

## examples/mini-sites/

- `desktop.ini` (~29 tok)
- `README.md` — Project documentation (~14 tok)

## examples/small-guides/

- `desktop.ini` (~29 tok)
- `README.md` — Project documentation (~16 tok)

## media/

- `desktop.ini` (~29 tok)
- `youtube-links.md` — YouTube Links (~46 tok)

## start-here/

- `desktop.ini` (~29 tok)
- `README.md` — Project documentation (~347 tok)
